...
export default class SignInForm extends React.Component {
	...
	state = { error: null, buttonDisabled: false };
	...
	handleSubmitJwtAuth = (ev) => {
		...
		AuthApiService.postLogin({
			...
			.catch((res) => {
				this.setState({ error: res.error, buttonDisabled: false });
			});
	};

	render() {
		const { error } = this.state;
		let buttonFunc = this.state.buttonDisabled ? 'Verifying...' : 'Sign In';
		return (
			<section role="contentinfo">
				...
				<form onSubmit={(e) => this.handleSubmitJwtAuth(e)}>
					...
					<br />
					<span id="user-password-span">*user name/password are case sensitive</span>
					<br />
					<button id="sign-in-button" disabled={this.state.buttonDisabled}>
						{buttonFunc}
					</button>
				</form>
